...
**Phases**（仅供参考，实际由脚本动态决定）: Scope → Find → Verify → Synthesize
**Source**: `/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/bughunt-lite.js` (builtin)
...
> 由 `sync-commands.js` 在 session_start 时从 `/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/bughunt-lite.js` grep `args.XXX` 而得。若以下字段名跟你预期不符，说明 workflow 源码已变，重启 coco session 会自动刷新此命令。

## Step 2 — 把 `$ARGUMENTS` 转成 JSON

(此 workflow 不读 `args`，无参 `{}`)
...
```bash
COCO_WF_BACKEND=traex node "/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/runtime/cli.js" run "bughunt-lite" --args='<把 Step 2 的 JSON 嵌这里>' --no-tui --events-ndjson 2>&1
```

`run_in_background: true`。然后**等约 2 秒**一次 `BashOutput` 拉前几行，从 `workflow:start` 事件里抓 `wfid` 字段（这是 workflow run id）。

> Runtime 绝对路径已硬编码：`/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/runtime/cli.js` —— Step 5 输出 TUI 命令时直接复用，不需要再去拿 PLUGIN_ROOT。
...
- `subject`: `workflow: bughunt-lite (<wfid>)`  ← **wfid 必填**
- `activeForm`: `Running bughunt-lite (<wfid>)`
- `description`: `Lighter bug sweep — fixed 3-rapid+2-deep finders stream into 5-vote adversarial verification (pigeon...` + `\nwfid=<wfid>` + `\nargs=<你构造的 JSON>` + `\n实时进度: /coco-workflow:workflows`
...
交互 TUI（另开终端）: node /Users/bytedance/Library/Caches/coco/plugins/coco-workflow/runtime/cli.js watch <wfid>
...
<!-- coco-workflow:auto-generated source=/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/bughunt-lite.js scope=builtin -->
