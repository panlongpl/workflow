...
**Phases**（仅供参考，实际由脚本动态决定）: Discover → Outline → Write → Verify → PR
**Source**: `/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/docs.js` (builtin)
...
> 由 `sync-commands.js` 在 session_start 时从 `/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/docs.js` grep `args.XXX` 而得。若以下字段名跟你预期不符，说明 workflow 源码已变，重启 coco session 会自动刷新此命令。
...
```bash
COCO_WF_BACKEND=traex node "/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/runtime/cli.js" run "docs" --args='<把 Step 2 的 JSON 嵌这里>' --no-tui --events-ndjson 2>&1
```
...
- `subject`: `workflow: docs (<wfid>)`  ← **wfid 必填**
- `activeForm`: `Running docs (<wfid>)`
- `description`: `Documentation generator. Discovers the feature surface and existing doc conventions, outlines for th...` + `\nwfid=<wfid>` + `\nargs=<你构造的 JSON>` + `\n实时进度: /coco-workflow:workflows`
...
<!-- coco-workflow:auto-generated source=/Users/bytedance/Library/Caches/coco/plugins/coco-workflow/workflows/docs.js scope=builtin -->
